...
function stonePick(arr) {
    ...
    let cobblestone = arr.filter((word) => word === 'Cobblestone').length;
      
    let sticks = 0;
    let sticksTotal = arr.reduce(
      (accumulator, currentValue) => {
      if (currentValue  === 'Sticks'){
      sticks += 1 
    } else if s(currentValue  === 'Wood'){
        sticks += 4 
      }
        },
      sticks
      ...
      let setCobblestone = Math.floor(cobblestone / 3)
      let setSticks = Math.floor(sticks / 2)
      
     return Math.min(setCobblestone, setSticks)
    }
