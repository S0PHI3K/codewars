...
function match(candidate, job) {
    ...
    const updatedSalary = (candidate.minSalary / 100) * 10;
    const compensation = updatedSalary + job.maxSalary ;
  
    
   if (candidate.minSalary  <= job.maxSalary) {
      return true
   } else if (candidate.minSalary <= compensation) {
      return true
     } else if (candidate.minSalary > job.maxSalary) {
      return false
     } else {
   throw new Error('No salary provided')
   }
